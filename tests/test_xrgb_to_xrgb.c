...
static void fill_image(gfx_blit_image_t *img, uint32_t width, uint32_t height, void* ptr) {
    ...
    for (uint32_t y = 0; y < height; ++y) {
        for (uint32_t x = 0; x < width; ++x) {
            uint8_t blue_val = (y * 255) / height;
            pixels[y * (img->stride[0] / 4) + x] = 0xFF000000 | (blue_val << 0); // XRGB
        }
    }
}
...
int main() {
    ...
    void *dst_ptr = create_image(dst, 1024, 1024, GFX_FORMAT_XRGB8888);
    if (!dst_ptr) {
    ...
    write_to_file("output.xrgb", dst, dst_ptr);
